...
if (!class_exists('MultiPostThumbnails')) {
	...
	class MultiPostThumbnails {
		...
		public function get_meta_key() {
			return "_{$this->post_type}_{$this->id}_thumbnail_id";
		}
...
//				wp_enqueue_script( "mpt-featured-image", $this->plugins_url( 'js/multi-post-thumbnails-admin.js', __FILE__ ), array( 'jquery', 'media-upload' ) );
				wp_enqueue_script( "mpt-featured-image", plugins_url( 'js/libs', __FILE__ ) . '/multi-post-thumbnails-admin.js', array( 'jquery', 'media-upload' ) );
			} else { // 3.5+ media modal
				wp_enqueue_media();
				wp_enqueue_script( "mpt-featured-image", plugins_url( 'js/libs', __FILE__ ) . '/multi-post-thumbnails-admin.js', array( 'jquery', 'set-post-thumbnail' ) );
				wp_enqueue_script( "mpt-featured-image-modal", plugins_url( 'js/libs', __FILE__ ) . '/media-modal.js', array( 'jquery', 'media-models' ) );				
			}
		}
		...
		public static function get_post_thumbnail_id($post_type, $id, $post_id) {
			return get_post_meta($post_id, "_{$post_type}_{$id}_thumbnail_id", true);
		}
		...
		public static function set_meta($post_ID, $post_type, $thumbnail_id, $thumbnail_post_id) {
			return update_post_meta($post_ID, "_{$post_type}_{$thumbnail_id}_thumbnail_id", $thumbnail_post_id);
		}
